% AATS Optimization for the Two-sided 4 Regions VSI Run Sum Xbar Chart %
% Subject to AATS0 and ASI0 %
% Gamma Distribution %
% Steady State %
...
for S1 = 0:1
    for S2 = S1:3
        for S3 = S2:5 
            for S4 = S3:10
                [hL, K] = FindhLKVsiRSXbarGamSteady(AATS0_target, ASI0_target, n, hS, G, S1, S2, S3, S4, a_shape, b_scale); 
                AATS0 = AATSFindGam(n, K, 0, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
                ASI0 = ASIFindGam(n, K, 0, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ;
                if abs(AATS0 - AATS0_target) < 1 
                    AATS1 = AATSFindGam(n, K, delta, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
                    SDATS1 = SDATSFindGam(n, K, delta, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
                    out = [out; hS, hL, S1, S2, S3, S4, K, AATS0, ASI0, AATS1, SDATS1] 
                    if AATS1 < AATS1min
                        SDATS1min = SDATS1 ; 
                        AATS1min = AATS1 ;
                        AATS0min = AATS0 ; 
...
disp(['The AATS1 is              : ', num2str(AATS1min,'%0.2f')])
disp(['The SDATS1 is             : ', num2str(SDATS1min,'%0.2f')])
disp(['The AATS0 is              : ', num2str(AATS0min,'%0.2f')])
