% ATS Optimization for the Two-sided 4 Regions VSI Run Sum Xbar Chart %
...
for S1 = 0:1
    for S2 = S1:3
        for S3 = S2:5 
            for S4 = S3:10
                [hL, K] = FindhLKVsiRSXbarGam(ATS0_target, ASI0_target, n, hS, G, S1, S2, S3, S4, a_shape, b_scale); 
                ...
                if abs(ATS0 - ATS0_target) < 1 
                    ATS1 = ATSFindGam(n, K, delta, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
                    SDTS1 = SDTSFindGam(n, K, delta, hS, hL, G, S1, S2, S3, S4, a_shape, b_scale) ; 
                    out = [out; hS, hL, S1, S2, S3, S4, K, ATS0, ASI0, ATS1, SDTS1] 
                    if ATS1 < ATS1min
                        SDTS1min = SDTS1 ; 
                        ATS1min = ATS1 ;
                        ATS0min = ATS0 ; 
